...
(function () {
  ...
  function caseCardHTML(t) {
    ...
    // 能力标签：取核心能力的中文名称（避免英文缩写），最多 3 个
    const tags = (t.capabilities || [])
      .map((c) => c.name || c.key)
      .filter(Boolean)
      .slice(0, 3);
    return `
      <div class="tcard" data-id="${escape(t.id)}">
        ...
        ${
          tags.length
            ? `<div class="tcard-caps">${tags
                .map((k) => `<span class="tcard-cap">${escape(k)}</span>`)
                .join('')}</div>`
            : ''
        }
  ...
  function showCaseDetail(id) {
    ...
    const hasKpis = t.kpis && t.kpis.length;
    const hasMetrics = t.metrics && t.metrics.length;
    if (hasKpis || hasMetrics) {
      const kpiCards = hasKpis
        ? t.kpis
            .map(
              (k) => `
            <div class="metric-card">
              <div class="metric-num ${k.trend || ''}">${escape(k.value)}</div>
              <div class="metric-label">${escape(k.label)}</div>
            </div>`
            )
            .join('')
        : '';
      ...
      html += `
        <div class="block">
          <div class="block-head">
            ...
            <span class="block-sub">核心 KPI 与运行时关键指标</span>
          </div>
          <div class="metric-grid">${kpiCards}${metricCards}</div>
        </div>`;
    }
    ...
    if (t.demo && t.demo.presets && t.demo.presets.length) {
      bindDemo(t.demo.presets, t.audience);
    }
  ...
  // ============================================================
  // 行业页：Tab Bar 筛选式（A 案）
  //  - 一条 Bar 平铺所有行业，点选即切换，无需「钻取 -> 看完 -> 返回」
  //  - 选中后用一条轻量「语境条」呈现该行业的定位 + 典型场景
  //  - 下方平铺该行业的相关案例卡片
  function renderIndustries(industryId) {
    const inds = state.data.industries || [];
    if (!inds.length) return;

    // 选中行业：优先显式传入 > 已有状态 > 第一个
    let activeId = industryId || state.activeIndustryId;
    if (!activeId || !getIndustry(activeId)) activeId = inds[0].id;
    state.activeIndustryId = activeId;

    // 1) Tab Bar
    const tabs = $('#industryTabs');
    if (tabs) {
      tabs.innerHTML = inds
        .map((ind) => {
      ...
      tabs.onclick = (e) => {
        ...
        if (btn && btn.dataset.ind !== state.activeIndustryId) renderIndustryTab(btn.dataset.ind);
      };
    }

    renderIndustryTab(activeId, true);
  }

  // 渲染选中行业的语境条 + 案例（Tab 切换时只更新这部分，不滚动到顶）
  function renderIndustryTab(id, isInitial) {
    const ind = getIndustry(id);
    if (!ind) return;
    state.activeIndustryId = id;

    // 同步 Tab 选中态
    $$('#industryTabs .ind-tab').forEach((b) => {
      const on = b.dataset.ind === id;
    ...
    if (ctx) {
      ctx.innerHTML = `
        <div class="ind-ctx-main">
          <div class="ind-ctx-name">${escape(ind.name)}行业大模型解决方案</div>
          <div class="ind-ctx-tag">${escape(ind.tagline)}</div>
        </div>
        ${
          (ind.scenarios || []).length
            ? `<div class="ind-ctx-scenes">
                <span class="ind-ctx-scenes-label">典型场景</span>
                ${(ind.scenarios || [])
                  .map((s) => `<span class="ind-scene">${escape(s)}</span>`)
                  .join('')}
              </div>`
            : ''
        }`;
    ...
    // 相关案例平铺
    const cases = (ind.relatedCases || []).map((cid) => getTemplate(cid)).filter(Boolean);
    const wrap = $('#industryCases');
    if (wrap) {
      wrap.innerHTML = cases.length
        ? cases.map((t) => caseCardHTML(t)).join('')
        : `<div class="placeholder-box">
  ...
  function bindDemo(presets, audience) {
    const presetEls = $$('#caseDetail .preset-item');
    const box = $('#demoMessages');
    // 对客场景（如导诊、智能助教）：智能体直接面向终端用户，更像「一个人」在对话，
    // 不展示意图识别/检索/工具调用等内部思考过程；对内场景仍完整呈现编排轨迹。
    ...
    async function show(idx, force) {
      ...
      const token = await playPreset(presets[idx], isCustomer);
  ...
  // 内部「思考过程」步骤类型（意图识别/规划/工具/检索/多模态/推理/动作/输出）
  const PROCESS_STEP_TYPES = ['input', 'intent', 'plan', 'tool', 'rag', 'multimodal', 'llm', 'action', 'output'];

  async function playPreset(preset, isCustomer) {
    if (!preset) return -1;
    clearDemoTimers();
    ...
    for (let i = 0; i < preset.steps.length; i++) {
      ...
      // 对客场景：跳过内部思考过程步骤，只用一个「正在输入」气泡，让客服更像真人
      if (isCustomer && PROCESS_STEP_TYPES.includes(step.type)) {
        if (!typingShown) {
          typingShown = true;
          const typing = appendTyping(box);
          await sleep(900 + Math.random() * 500);
          if (myToken !== cancelToken) { typing.remove(); return myToken; }
          typing.remove();
        }
        continue;
      }
  ...
  // 对客场景的「正在输入」气泡（拟人化，无内部思考过程）
  function appendTyping(box) {
    const div = document.createElement('div');
    div.className = 'msg bot';
    div.innerHTML = `${BOT_AVATAR}<div class="msg-typing"><i></i><i></i><i></i></div>`;
    box.appendChild(div);
    scrollToBottom(box);
    return div;
  }
  ...
  function navigate(section, opts = {}) {
    ...
    if (section === 'home') {
      ...
      renderIndustries(opts.industryId);
    }
  ...
  // 场景配置：每个场景一句话任务 + 一条多步执行轨迹
  // type: plan(规划) / rag(检索知识库) / tool(调用工具) / rule(规则判定) / deliver(交付物)
  const ADP_TRY_SCENES = [
    {
      id: 'return',
      label: '客户退货能否受理',
      prompt: '客户要退这台路由器，能不能受理？帮我判断并处理',
      why: '需查售后政策库 + 调订单系统 API + 按规则判定 + 自动建工单',
      think: '用户想退一台路由器，这是售后受理判断，不能凭感觉答复。我得先检索售后政策库确认「无理由退货」的时效要求，再调订单系统拿到真实签收时间，两者一比对就能判定该走哪条流程，最后落成工单并通知客户。',
      steps: [
        { type: 'plan', title: '规划智能体 · 任务拆解', body: '拆为 5 步：检索售后政策 → 查订单签收信息 → 规则判定 → 生成结论 → 建工单并通知。' },
        { type: 'rag', title: '检索企业知识库', body: '命中《售后服务政策 v3.2》第 4.2 条：7 天无理由退货，签收满 7 天需走质量/三包流程。', meta: '来源：售后政策库 · 相似度 0.93' },
        { type: 'tool', title: '调用工具 OrderAPI', body: '入参 {orderId:"88231"} → 返回：签收 45 天，状态「已签收」，商品「企业级路由器」。', meta: 'GET /orders/88231 · 200 OK · 142ms' },
        { type: 'rule', title: '规则判定', body: '签收 45 天 > 7 天，不适用「无理由退货」；转入「三包/质量问题」判定流程，需客户补充故障描述。' },
        { type: 'deliver', title: '生成交付物', body: '已生成处理结论与工单。', deliver: { name: '退货处理工单 #RT-88231', items: ['结论：不适用无理由退货，转质量检测', '已建工单并指派售后二线', '已向客户发送补充故障说明的短信'] } },
      ],
    },
    {
      id: 'contract',
      label: '合同条款合规审查',
      prompt: '审一下这份采购合同，有没有不合规或高风险条款？',
      why: '需读合同正文 + 比对企业合规条款库 + 逐条标注 + 出风险报告入库',
      think: '这是一份采购合同的合规审查。我不能只凭经验判断，得先把合同正文逐条解析出来，再拿企业的合规红线库去比对，命中红线的条款按风险等级标注，最后汇总成一份可归档、可追责的风险报告。',
      steps: [
        { type: 'plan', title: '规划智能体 · 任务拆解', body: '拆为 5 步：解析合同条款 → 比对合规库 → 逐条风险标注 → 汇总评级 → 生成报告并归档。' },
        { type: 'tool', title: '调用工具 文档解析', body: '解析《采购框架合同.pdf》→ 抽取 28 条条款、3 个金额字段、2 个违约责任段落。', meta: 'POST /doc/parse · 28 clauses' },
        { type: 'rag', title: '检索合规条款库', body: '比对企业《合同合规红线 v5》：命中「付款账期 ≤ 60 天」「必须含数据安全条款」「单方解约需双向对等」。', meta: '来源：法务合规库 · 命中 3 条红线' },
        { type: 'rule', title: '逐条风险标注', body: '第 7 条账期 90 天 → 高风险（超红线）；第 19 条缺数据安全条款 → 中风险；第 22 条解约权不对等 → 中风险。' },
        { type: 'deliver', title: '生成交付物', body: '已生成合规风险报告并归档。', deliver: { name: '合同合规风险报告', items: ['整体评级：中高风险（1 高 / 2 中）', '第7条建议改为账期≤60天', '建议补充数据安全与解约对等条款', '已归档至法务知识库并通知承办人'] } },
      ],
    },
    {
      id: 'restock',
      label: '智能补货下单',
      prompt: '看看哪些 SKU 快缺货了，需要的话直接补货下单',
      why: '需查实时库存 API + 读历史销量 + 算补货量 + 调采购下单 API + 发通知',
      think: '补货不是看哪个少就补哪个。我得先用库存 API 找出低于安全库存的 SKU，再结合历史销量和补货策略算出合理的补货量，避免压货或断货，确认后再调采购下单 API，并通知相关负责人。',
      steps: [
        { type: 'plan', title: '规划智能体 · 任务拆解', body: '拆为 5 步：查实时库存 → 拉历史销量 → 计算补货量 → 调采购下单 → 通知采购负责人。' },
        { type: 'tool', title: '调用工具 InventoryAPI', body: '返回 3 个低于安全库存的 SKU：A-102 剩 12 / B-330 剩 5 / C-771 剩 0（缺货）。', meta: 'GET /inventory/low-stock · 3 items' },
        { type: 'rag', title: '检索历史销量与补货策略', body: '读取近 30 天销量与《补货策略》：按「均日销 ×（采购周期+安全冗余）」测算建议补货量。', meta: '来源：销量数仓 + 运营策略库' },
        { type: 'tool', title: '调用工具 PurchaseAPI', body: '提交采购单：A-102×200、B-330×150、C-771×300（合计 ¥86,400）。', meta: 'POST /purchase/orders · 201 Created' },
        { type: 'deliver', title: '生成交付物', body: '已下单并通知相关负责人。', deliver: { name: '采购单 #PO-20260625', items: ['3 个 SKU 已自动补货下单', '预计 3 个工作日到货', '已企业微信通知采购与仓储负责人', '同步更新库存看板'] } },
      ],
    },
  ];
  ...
  const TRY_STEP_META = {
    ...
    tool: { icon: '🔧', tag: '调用工具' },
    rule: { icon: '⚖', tag: '规则判定' },
    deliver: { icon: '📄', tag: '生成交付物' },
  };

  function renderTryStep(step) {
    const meta = TRY_STEP_META[step.type] || TRY_STEP_META.plan;
    let extra = '';
    if (step.meta) {
      extra += `<code class="adp-step-meta">${escape(step.meta)}</code>`;
    }
    if (step.deliver) {
      const items = (step.deliver.items || []).map((t) => `<li>${escape(t)}</li>`).join('');
  ...
  // 交付物结果卡（始终可见，不随思考过程折叠）
  function renderTryResult(step) {
    const items = ((step.deliver && step.deliver.items) || []).map((t) => `<li>${escape(t)}</li>`).join('');
    const name = (step.deliver && step.deliver.name) || step.title;
    return `
      <div class="adp-result-card">
        <div class="adp-result-flag"><span class="adp-result-ico">✓</span>交付物已生成</div>
        <div class="adp-result-name">${escape(name)}</div>
        ${items ? `<ul class="adp-result-items">${items}</ul>` : ''}
      </div>`;
  }

  // 绑定「思考过程」标题的展开/折叠交互（点击切换）
  function bindThinkToggle(trace) {
    const think = $('.adp-think', trace);
    if (!think) return;
    const head = $('.adp-think-head', think);
    if (!head || head.dataset.bound) return;
    head.dataset.bound = '1';
    head.addEventListener('click', () => {
      const collapsed = think.getAttribute('data-collapsed') === 'true';
      think.setAttribute('data-collapsed', collapsed ? 'false' : 'true');
  ...
  function setupHeroTry() {
    const root = $('#adpTry');
    if (!root) return;
    const scenesBox = $('#adpTryScenes', root);
    const trace = $('#adpTryTrace', root);
    if (!scenesBox || !trace) return;

    let activeId = ADP_TRY_SCENES[0].id;
    let playToken = 0;            // 每次播放自增，旧定时器据此失效，避免快速切卡时串场
    const sceneCache = {};        // 已播放完成的场景缓存其最终 HTML，再次切换不重播

    // 构建轨迹骨架：思考过程 + 执行步骤统一收进可折叠区块，交付物作为结果常驻展示
    const buildTrace = (scene) => `
      ...
      <div class="adp-think" data-collapsed="false">
        ...
        <div class="adp-think-body"><div class="adp-think-inner">
          <p class="adp-think-text"></p>
          <div class="adp-trace-list"></div>
        </div></div>
      </div>
      <div class="adp-result" aria-live="polite"></div>
      <div class="adp-trace-loading"><i></i><i></i><i></i><em>ADP 正在编排执行…</em></div>`;
    ...
    const playScene = (scene) => {
      ...
      // 已缓存：直接渲染完成态，思考与执行过程默认折叠，不再重播
      if (sceneCache[scene.id]) {
        trace.innerHTML = sceneCache[scene.id];
        root.classList.remove('is-running');
        root.classList.add('is-done');
        bindThinkToggle(trace);
        return;
      }

      trace.innerHTML = buildTrace(scene);
      bindThinkToggle(trace);

      const think = $('.adp-think', trace);
      const thinkText = $('.adp-think-text', think);
      const thinkTitle = $('.adp-think-title', think);
      const head = $('.adp-think-head', think);
      const list = $('.adp-trace-list', trace);
      const result = $('.adp-result', trace);

      const procSteps = scene.steps.filter((s) => s.type !== 'deliver');
      const deliverStep = scene.steps.find((s) => s.type === 'deliver');

      // 1) 思考过程：逐字打印
      const full = scene.think || '';
      const charDelay = 22;
      const thinkStart = 320;
      for (let i = 0; i < full.length; i++) {
        const t = setTimeout(() => {
          if (myToken !== playToken) return;
          thinkText.textContent = full.slice(0, i + 1);
        }, thinkStart + i * charDelay);
        heroTryTimers.push(t);
      }
      const afterThink = thinkStart + full.length * charDelay + 240;

      // 2) 思考完成 → 进入执行，逐条展示步骤（仍在可折叠区内）
      const tExec = setTimeout(() => {
        if (myToken !== playToken) return;
        thinkTitle.textContent = '正在执行…';
      }, afterThink);
      heroTryTimers.push(tExec);

      const gap = 680;
      const stepBase = afterThink + 200;
      procSteps.forEach((step, idx) => {
        const timer = setTimeout(() => {
          if (myToken !== playToken || !list) return;
          list.insertAdjacentHTML('beforeend', renderTryStep(step));
          const last = list.lastElementChild;
          if (last) last.classList.add('adp-step-in');
        }, stepBase + idx * gap);
        heroTryTimers.push(timer);
      });

      // 3) 交付物结果出现（常驻展示）
      const resultAt = stepBase + procSteps.length * gap;
      const tResult = setTimeout(() => {
        if (myToken !== playToken || !result || !deliverStep) return;
        result.innerHTML = renderTryResult(deliverStep);
        const card = result.firstElementChild;
        if (card) requestAnimationFrame(() => card.classList.add('adp-result-in'));
      }, resultAt);
      heroTryTimers.push(tResult);

      // 4) 全部完成：把「思考 + 执行过程」整体折叠起来，仅留结果 + 可点击展开
      const collapseAt = resultAt + 700;
      const done = setTimeout(() => {
        if (myToken !== playToken) return;
        thinkTitle.textContent = `思考与执行过程 · 共 ${procSteps.length} 步`;
        think.classList.add('is-done');
        think.setAttribute('data-collapsed', 'true');
        if (head) head.setAttribute('aria-expanded', 'false');
        root.classList.remove('is-running');
        root.classList.add('is-done');
        const loading = $('.adp-trace-loading', trace);
        if (loading) loading.remove();
        $$('.adp-step', list).forEach((el) => el.classList.add('adp-step-in'));
        sceneCache[scene.id] = trace.innerHTML;
      }, collapseAt);
      heroTryTimers.push(done);
    };

    const selectScene = (id) => {
      const scene = ADP_TRY_SCENES.find((s) => s.id === id) || ADP_TRY_SCENES[0];
      activeId = scene.id;
      $$('.adp-scene', scenesBox).forEach((el) => {
        el.classList.toggle('active', el.dataset.scene === scene.id);
        el.setAttribute('aria-selected', el.dataset.scene === scene.id ? 'true' : 'false');
      });
      playScene(scene);
    };

    scenesBox.innerHTML = ADP_TRY_SCENES.map((s) => `
      <button type="button" class="adp-scene${s.id === activeId ? ' active' : ''}" role="tab" data-scene="${s.id}" aria-selected="${s.id === activeId}">
        <b>${escape(s.label)}</b>
        <small>${escape(s.why)}</small>
      </button>`).join('');

    scenesBox.addEventListener('click', (e) => {
      const btn = e.target.closest('.adp-scene');
      if (btn && btn.dataset.scene !== activeId) selectScene(btn.dataset.scene);
    });
